...
<table class="table table-hover">
  ...
  <tbody>
    <?php
      ...
      $bd = new connexio();
      $consulta = $bd->query("SELECT ID FROM Usuaris  WHERE Nom LIKE '%".$criteri."%' OR Cognom1 LIKE '%".$criteri."%' OR Cognom2 LIKE '%".$criteri."%'");
      ...
      if ($ini<0) { $ini=0;}
      $fi = $perpagina;
      $consulta2 = $bd->query("SELECT ID,Nom,Cognom1,Cognom2,DNI,Alta_Baixa FROM Usuaris WHERE Nom LIKE '%".$criteri."%' OR Cognom1 LIKE '%".$criteri."%' OR Cognom2 LIKE '%".$criteri."%' ORDER BY Cognom1,Cognom2,Nom LIMIT $ini,$fi");
      $i=1;
